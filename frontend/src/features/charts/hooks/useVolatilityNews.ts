...
export const useVolatilityNews = ({ 
  ...
  const fetchVolatilityData = async () => {
    ...
    try {
      ...
      for (const symbol of validSymbols) {
        try {
          const volatilityEvents = await chartsApiService.getStockVolatilityNews(symbol, startDate, endDate);
          results[symbol] = volatilityEvents;
        } catch (symbolError) {
  ...
  const openNewsModal = async (_date: string, event: VolatilityEvent) => {
    ...
    try {
      ...
      const newsResponse = await chartsApiService.searchNews(companyName, 15);
      console.log(`${companyName}에 대한 뉴스 ${newsResponse.items.length}개를 찾았습니다.`);
