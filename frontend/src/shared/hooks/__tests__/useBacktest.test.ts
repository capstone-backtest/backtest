...
import { backtestApiService } from '../../features/backtest/services'
...
describe('useBacktest', () => {
  ...
  it('성공 시 결과를 설정하고 isPortfolio를 올바르게 표시해야 함', async () => {
    ;(backtestApiService.runBacktest as any).mockResolvedValue({ ok: true })
  ...
  it('네트워크 오류를 매핑해 errorType과 메시지를 설정해야 함', async () => {
    ;(backtestApiService.runBacktest as any).mockRejectedValue({
      message: '네트워크 오류',
      status: 0,
      type: 'network',
      errorId: undefined,
  ...
  it('알 수 없는 오류는 unknown으로 처리해야 함', async () => {
    ;(backtestApiService.runBacktest as any).mockRejectedValue(new Error('Boom'))
