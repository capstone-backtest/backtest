...
import { ChartDataPoint, EquityPoint, TradeMarker } from '../../../types/api';
import { formatChartDate, formatDate } from '../common/dateUtils';
import { formatPercent, formatPrice, getColorByValue } from '../common/numberUtils';

// 차트 데이터 변환
export const transformOHLCData = (data: ChartDataPoint[]) => {
...
export const transformTradeMarkers = (trades: TradeMarker[]) => {
  return trades.map(trade => ({
    ...
    displayPrice: formatPrice(trade.price),
    displayPnl: trade.pnl_pct ? formatPercent(trade.pnl_pct) : 'N/A',
